...
function deleteRole(){
	...
	$.messager.confirm("确认", "您确认删除选定的记录吗？", function (deleteAction) {
		if (deleteAction) {
			showLoading();
			//请求参数 ?ids[]=11&ids[]=12
			/*Public.ajaxGet('${pageContext.request.contextPath}/role/delete', {ids : ids}, function(e) {
				hideLoading();
				if (200 == e.status) {
			...
			//请求体的json数据： [2,3,10,15]   Contorller中  @RequestBody List<Integer> list
			Public.ajaxPost("${pageContext.request.contextPath}/role/delete",JSON.stringify(ids),function(e) {
                hideLoading();
                if (200 == e.status) {
                    $.messager.alert('提示','操作成功。','info');
                    $('#roleform').submit();
