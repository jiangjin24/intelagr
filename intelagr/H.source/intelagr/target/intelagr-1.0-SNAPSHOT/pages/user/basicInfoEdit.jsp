...
function save(){
	...
	//$('#fm') 获得表单对象（jquery对象）
	//$('#fm').toObject(); 将表单中表单元素的名字和值 构造成 {}对象
	//$('#fm').toObject(); 的结果就是  {userName:"张国强",userID:"zgq",tel:"13333333",email:"aaa"}
	var a = $('#fm').toObject();
	//发送了一个ajax方法   请求内容类型是application/json   将{userName:"张国强",userID:"zgq",tel:"13333333",email:"aaa"}放在http请求的报文体中
	// 发给了Controller  由于Controller方法参数有@RequestBody注解修饰  SpringMVC先读取请求内容类型(application/json)，调用json的解析器对http请求报文体的数据  解析到修饰的参数中
	Public.ajaxPost("${pageContext.request.contextPath}/user/update",JSON.stringify(a),
        function(e){
            if (200 == e.status) {
