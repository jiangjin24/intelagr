			...
			<form id="addroleform" action="${pageContext.request.contextPath}/role/addRole" class="easyui-form" method="post" data-options="novalidate:true">
				...
				<table class="table_common_style">
	    			...
	    			<tr>
		    			...
		    			<td class="table_common_td_txt_style">
		    				<textarea  class="easyui-textbox" rows="2" id="remark" name="remark" value="${role.remark}" style="width:200px;height:60px" data-options="multiline:true" ></textarea>
		    			</td>
	    			</tr>
	    			<tr height="50">
...
function addForm_check()
{
	...
	var a = $('#addroleform').toObject();   //获得一个Object对象
	showLoading();
	//Controller的参数  直接Role role
	Public.ajaxGet("${pageContext.request.contextPath}/role/update",a,function(e){
        hideLoading();
        if (200 == e.status) {
	...
	//Controller 的方法参数  @RequestBody Role role
	/*Public.ajaxPost("${pageContext.request.contextPath}/role/save",JSON.stringify(a),function(e){
        hideLoading();
        if (200 == e.status) {
            $.messager.alert('提示','操作成功。','info');
            $('#addDialog').window('close');
